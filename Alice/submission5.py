...
train_df_newfeatures['len1'] = (train_df[times].count(axis = 1) == 1) * 1 - 0.5
test_df_newfeatures['len1'] = (test_df[times].count(axis = 1) == 1) * 1 - 0.5

train_df_newfeatures['len2'] = (train_df[times].count(axis = 1) == 2) * 1 - 0.5
test_df_newfeatures['len2'] = (test_df[times].count(axis = 1) == 2) * 1 - 0.5

train_df_newfeatures['len3'] = (train_df[times].count(axis = 1) == 3) * 1 - 0.5
test_df_newfeatures['len3'] = (test_df[times].count(axis = 1) == 3) * 1 - 0.5

train_df_newfeatures['len4'] = (train_df[times].count(axis = 1) == 4) * 1 - 0.5
test_df_newfeatures['len4'] = (test_df[times].count(axis = 1) == 4) * 1 - 0.5

train_df_newfeatures['len5'] = (train_df[times].count(axis = 1) == 5) * 1 - 0.5
test_df_newfeatures['len5'] = (test_df[times].count(axis = 1) == 5) * 1 - 0.5

train_df_newfeatures['len6'] = (train_df[times].count(axis = 1) == 6) * 1 - 0.5
test_df_newfeatures['len6'] = (test_df[times].count(axis = 1) == 6) * 1 - 0.5

train_df_newfeatures['len7'] = (train_df[times].count(axis = 1) == 7) * 1 - 0.5
test_df_newfeatures['len7'] = (test_df[times].count(axis = 1) == 7) * 1 - 0.5

train_df_newfeatures['len8'] = (train_df[times].count(axis = 1) == 8) * 1 - 0.5
test_df_newfeatures['len8'] = (test_df[times].count(axis = 1) == 8) * 1 - 0.5

train_df_newfeatures['len9'] = (train_df[times].count(axis = 1) == 9) * 1 - 0.5
test_df_newfeatures['len9'] = (test_df[times].count(axis = 1) == 9) * 1 - 0.5

#    Day of week
train_df_newfeatures['year'] = train_df['time1'].apply(lambda ts : ts.date().year())
test_df_newfeatures['year'] = test_df['time1'].apply(lambda ts : ts.date().year())
...
new_features = ['dow_scaled', 'year_scaled','active_days', 'active_hours', 'sesslen_scaled']

X_train_new = csr_matrix(hstack([X_train, train_df_newfeatures[new_featres]]))
